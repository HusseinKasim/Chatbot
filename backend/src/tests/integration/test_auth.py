...
# Test case: Test_User_Login_Empty_Password
def test_user_login_empty_password(db, sample_user):
    app.dependency_overrides[get_db] = lambda: db
    ...
    response = client.post('/api/auth/login', json={'email': sample_user['email'], 'password': EMPTY_PASSWORD})
